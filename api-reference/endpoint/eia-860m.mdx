...
curl -H "x-api-key: your_api_key_here" \
  "https://api.cleanview.co/api/v0/eia-860m?limit=50&state=CA&energy_source=SUN"
```

```javascript JavaScript
const response = await fetch('https://api.cleanview.co/api/v0/eia-860m?limit=50&state=CA&energy_source=SUN', {
  headers: {
...
| `energy_source` | string | - | Filter by energy source code(s), comma-separated (e.g., 'SUN', 'WND', 'MWH') |
| `balancing_authority` | string | - | Filter by balancing authority code(s), comma-separated (e.g., 'CISO', 'ERCO', 'PJM') |
| `status` | string | 'operating' | Filter by operational status |
| `start_year` | integer | - | Minimum operating year |
| `end_year` | integer | - | Maximum operating year |
...
<AccordionGroup>
  <Accordion title="Get Solar Projects in California">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/eia-860m?state=CA&energy_source=SUN&limit=50"
    ```
  ...
  <Accordion title="Get Large Wind Projects (>100 MW)">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/eia-860m?energy_source=WND&limit=50"
    ```
  ...
  <Accordion title="Get Battery Storage Projects">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/eia-860m?energy_source=MWH&status=operating&limit=50"
    ```
  ...
  <Accordion title="Get Projects by Multiple Balancing Authorities">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/eia-860m?balancing_authority=CISO,ERCO&limit=50"
    ```
