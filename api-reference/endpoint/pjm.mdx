...
api: 'GET https://api.cleanview.co/api/v0/interconnection/pjm'
...
| `state` | string | - | Filter by state code (PA, OH, NJ, etc.) |
...
| `transmissionOwner` | string | - | Filter by transmission owner |
...
<AccordionGroup>
  ...
  <Accordion title="Get Large Natural Gas Projects (>300 MW)">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/interconnection/pjm?technology=Natural Gas&minCapacity=300&limit=50"
    ```
  ...
  <Accordion title="Get Projects by Multiple States">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/interconnection/pjm?states=PA,OH,NJ&limit=50"
    ```
    
    Uses `states` parameter for multi-state filtering in the Mid-Atlantic region.
  </Accordion>
</AccordionGroup>
  ...
  "data": [
    {
      ...
      "projectNumber": "AB3-123",
      "commercialName": null,
      "maximumFacilityOutput": 200
    }
  ],
  "iso": "PJM",
  "pagination": {
