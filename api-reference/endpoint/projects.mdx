...
api: 'GET https://api.cleanview.co/api/v0/projects'
...
<Note>
**Data Sources**: This endpoint combines data from EIA 860M (operating plants) and interconnection queues from major ISOs/RTOs (planned projects).
</Note>
...
| `sourceTable` | string | - | Filter by data source table |
| `minCapacity` | number | - | Minimum capacity (MW) |
| `maxCapacity` | number | - | Maximum capacity (MW) |
| `balancingAuthority` | string | - | Filter by balancing authority code |
...
<AccordionGroup>
  ...
  <Accordion title="Get Large Projects (>100 MW)">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/projects?minCapacity=100&limit=50"
    ```
  </Accordion>
  ...
  <Accordion title="Get Projects from EIA Data Only">
    ```bash
    curl -H "x-api-key: your_api_key_here" \
      "https://api.cleanview.co/api/v0/projects?sourceTable=eia_860m&limit=50"
    ```
  </Accordion>
</AccordionGroup>
